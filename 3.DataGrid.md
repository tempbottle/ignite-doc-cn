...
在`分区`和`复制`缓存之间进行关联也没有什么限制。然而，如果在两个`分区`缓存间关联，一点要确保要关联的键是**并置**的。
...
例如，有一个`Person`和`Company`对象，然后希望将`Person`对象和其工作的`Company`对象并置在一起，为达到这个目的，缓存`Person`对象的实体需要有一个属性或者方法加注`@AffinityKeyMapped`注解，他提供了要并置的`Company`对象的键值。为了方便，也可以可选地使用`AffinityKey`类。
使用PersonKey:
```java
